...
| 3 | Ollama embeddings (`/api/embed`, `/api/tags` probe) | `semantic_index.py` (`OllamaBackend`) | HTTP via `urllib`; failover chain `192.168.4.75:11434` → `localhost:11434` (both `mxbai-embed-large`) | Per-host probe failure → next host; all dead → stdlib hashed backend; embed failure → `RuntimeError` → substring fallback in retrieval agent | Host failover + backend fallback | chosen host at INFO, probe failures at DEBUG, embed failures at ERROR via caller | Probe order + dead-host paths tested (mocked); live Ollama not tested in CI |
